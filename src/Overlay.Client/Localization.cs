...
public static class Localization
{
    ...
    private static readonly Dictionary<string, string> Ko = new()
    {
        ...
        ["stats.grade.gated"] = "표본이 얇아 한 단계 내려간 항목입니다. 점수는 컷을 넘지만, 표준오차를 감안한 하한이 {0}%p라 '50%를 넘는다'를 표본이 뒷받침하지 못합니다.",
        ...
        ["stats.grade.tip"] = "등급은 절대 기준입니다. 점수는 표본으로 보정한 승률이 50%보다 몇 %p 높은지이고, 컷은 S+ +4.0 / S +2.5 / A +1.0 / B -1.0 / C -3.0, 그 아래가 D입니다. 순위가 아니라 측정값으로 매기므로 라인·패치에 따라 S+가 없을 수도, 여럿일 수도 있습니다. 다만 S+·S는 표본이 '50%를 넘는다'를 실제로 뒷받침할 때만 줍니다 — 30판 60%는 점수가 +7.5여도 표본오차가 그보다 커서 A로 내려갑니다.",
        ["stats.filter.all"] = "전체",
    ...
    private static readonly Dictionary<string, string> En = new()
    {
        ...
        ["stats.grade.gated"] = "Held one grade down by its sample. The score clears the cutoff, but the conservative edge is {0} points, so the sample does not actually place this champion above even.",
        ...
        ["stats.grade.tip"] = "Grades are absolute. The score is how many points above 50% the sample-shrunk win rate sits, and the cutoffs are S+ +4.0 / S +2.5 / A +1.0 / B -1.0 / C -3.0, below which D. Because it is measured rather than ranked, a lane can have no S+ at all or several. The top two grades additionally require the sample to actually place the champion above even: a 30-game 60% scores +7.5 but its standard error is larger than that, so it lands at A.",
